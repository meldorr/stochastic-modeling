...
def plot_explained_variance(fpca, path):
    ...
    for ax, name, b in zip(np.atleast_1d(axes), fpca.feature_names, fpca.bases):
        ...
        ax.set_xlabel("components"); ax.set_xlim(0.5, min(len(cum), b["k"] + 8) + 0.5)
...
def plot_latent(real_W_n, gen_W_n, path):
    ...
    ax0.set_title("latent 2D PCA view"); ax0.set_xlabel("PC1"); ax0.set_ylabel("PC2")
    ...
    for j in range(ndim):
        ...
        ax.set_title(f"dim {j}", fontsize=8); ax.tick_params(labelsize=7)
...
def plot_spatial(real_ll, gen_ll, flows, path, n_show=400):
    ...
    ri = np.random.default_rng(0).choice(len(real_ll), min(n_show, len(real_ll)), replace=False)
    gi = np.random.default_rng(1).choice(len(gen_ll), min(n_show, len(gen_ll)), replace=False)
    ...
    axes[0].set_title(f"Real ({len(ri)})"); axes[1].set_title(f"Generated ({len(gi)})")
    for ax in axes:
        ax.set_xlabel("longitude"); ax.grid(alpha=0.3); ax.set_aspect("equal", "box")
...
def main() -> None:
    ...
    ridx = rng.choice(len(data["X"]), min(int(ecfg["n_real"]), len(data["X"])), replace=False)
    real_X = data["X"][ridx]
    ...
    glat, glon = walk_latlon_backward(gen_feats[:, :, ti], gen_feats[:, :, gi],
                                      gen_feats[:, :, tdi], gen_anchors)
    ...
    ks_latent = [float(ks_2samp(real_W_n[:, j], gen_W_n[:, j]).statistic)
                 for j in range(real_W_n.shape[1])]
    ks_feat = {name: float(ks_2samp(real_X[:, :, j].ravel(), gen_feats[:, :, j].ravel()).statistic)
               for j, name in enumerate(names)}
    metrics = {
        "n_real": len(real_X), "n_generated": len(gen_feats),
        "latent_dim_m": int(fpca.m),
        "components_per_feature": dict(zip(names, [int(k) for k in fpca.ks])),
        "explained_variance": {k: round(v, 4) for k, v in fpca.total_explained_variance().items()},
        "fpca_val_recon_rmse_std": {n: round(float(r), 4) for n, r in zip(names, recon_rmse)},
