...
def draw_batch(bundle, k, clamp):
    ...
    with np.errstate(all="ignore"):  # under-trained samples can overflow decode; expected
...
def sample_trajectories(bundle, n, reject, max_tries, clamp):
    ...
    while have < n and tries <= max_tries:
        ...
        last = (W, feats)
        mask = within_bounds(feats, bounds, names) if reject else np.ones(len(feats), bool)
        if mask.any():
            W_keep.append(W[mask]); F_keep.append(feats[mask]); have += int(mask.sum())
        tries += 1

    if have == 0:
        print("[generate] WARNING: no samples passed the physics filter "
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Generate trajectories from a trained pipeline")
    ...
    W, feats = sample_trajectories(bundle, n, bool(gcfg["physics_reject"]),
                                   int(gcfg["reject_max_tries"]), clamp)
    ...
    np.savez_compressed(npz, feats=feats.astype(np.float32), W=W.astype(np.float32),
