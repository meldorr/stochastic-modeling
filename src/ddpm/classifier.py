"""Noise-conditioned trajectory classifier for compositional classifier guidance.
...
gradient is one *term* in product-of-experts guidance (Dhariwal & Nichol 2021):
positive scale steers generation into the class, negative away from it (e.g. the
anti-loop expert).
"""
...
class TrajClassifier(nn.Module):
    ...
    so global shapes (loops, holdings, corridor membership) reach the pooled head.
    """
...
def load_classifier(path: str, device):
    ...
    a = ck.get("arch", {})
    clf = TrajClassifier(ck["channels"], ck["t_len"], width=a.get("width", 96),
...
# Backward-compat: the holding stages (s10/s11) on this machine import this name.
HoldingClassifier = TrajClassifier
