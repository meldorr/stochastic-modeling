...
Spatial source per feature set:
    xy        modelled x/y directly (absolute)
    gstrack   dead-reckoned from generated (track, gs, dt), start-centred
              (no absolute position exists -> shape-only comparison)
    controls  re-integrated tracks from Stage 2 (absolute; entry states real)
"""
...
from experiments.common import dead_reckon, ks_marginals, sliced_wasserstein
...
def spatial_paths(fs, feats, names, data, gen_npz, which):
    """(N, T, 2) x/y paths for 'real' or 'gen', plus whether they are centred."""
    if fs == "xy":
        if which == "real":
            xy = data["X"][data["val_idx"]][:, :, [names.index("x"), names.index("y")]]
        else:
            xy = feats[:, :, [names.index("x"), names.index("y")]]
        return xy.astype(np.float64), False
    if fs == "controls":
        if which == "real":
            return data["aux"]["real_xy"][data["val_idx"]].astype(np.float64), False
        return gen_npz["tracks_xyz"][:, :, :2].astype(np.float64), False
    # gstrack: dead-reckon, centred
    if which == "real":
        arr = data["X"][data["val_idx"]]
    else:
        arr = feats
    ti, gi, tdi = names.index("track"), names.index("groundspeed"), names.index("timedelta")
    z = np.zeros(len(arr))
    xy = dead_reckon(arr[:, :, ti], arr[:, :, gi], arr[:, :, tdi], z, z)
    return xy - xy[:, :1, :], True
...
def main() -> None:
    ...
    real_xy, centred = spatial_paths(fs, feats, names, data, g, "real")
    gen_xy, _ = spatial_paths(fs, feats, names, data, g, "gen")
    ...
    print(f"[{dirs['name']}] sliced-W(xy)={metrics['sliced_w_xy_km']:.3f} km  "
          ...
          f"within={metrics['within_pct'] * 100:.1f}%  centred={centred}")
    ...
    for ax, xy, ttl, c in ((axes[0], real_xy, f"Real ({len(real_xy)})", "#1f77b4"),
        ...
        ax.grid(alpha=0.3)
    fig.suptitle(f"{dirs['name']}: spatial ({'centred' if centred else 'absolute'})")
    fig.tight_layout()
